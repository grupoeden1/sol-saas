...
Segunda metade do pipeline: pega transcrição (AssemblyAI) + frames extraídos (FFmpeg) → Claude Vision (claude-sonnet-4-5-20250929) analisa cada frame visualmente → Claude (claude-sonnet-4-5-20250929) consolida tudo em uma descrição textual rica (ganchos, CTAs, estrutura, tom, técnicas de retenção). A `fullDescription` resultante é o que persiste no banco e alimenta o prompt de geração do roteiro.
...
| # | Critério | Status |
|---|----------|--------|
| 1 | Claude Vision (claude-sonnet-4-5-20250929) recebe frames extraídos e descreve conteúdo visual de cada um (o que aparece, cenário, ações, texto na tela) | TODO |
| 2 | Salva `frameDescriptions` no VideoAnalysis | TODO |
| 3 | Claude (claude-sonnet-4-5-20250929) recebe transcrição + frame descriptions e gera análise estrutural: ganchos usados, CTAs, estrutura do vídeo (intro, corpo, conclusão), tom de comunicação, técnicas de retenção, pontos fortes e fracos | TODO |
...
| 9 | Se análise Claude (claude-sonnet-4-5-20250929) falhar → marca FAILED com mensagem "Erro na análise do vídeo pela IA" | TODO |
...
- **Vision API:** Enviar frames como `image_url` (base64) para Claude (claude-sonnet-4-5-20250929) com prompt descritivo
- **Consolidação:** Prompt específico para Claude (claude-sonnet-4-5-20250929) que recebe transcrição + frame descriptions e gera análise estrutural
